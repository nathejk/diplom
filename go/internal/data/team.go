...
func (m TeamModel) GetPatruljeByYearAndNumber(year types.YearSlug, number string) (*Patrulje, error) {
	query := `SELECT p.teamId, p.teamNumber, p.name, p.groupName, p.korps, p.liga, p.memberCount, p.photoId
		FROM patrulje p
		WHERE p.year = ? AND p.teamNumber = ?`
	var p Patrulje
	err := m.DB.QueryRow(query, year, number).Scan(
		&p.ID,
		&p.Number,
		&p.Name,
		&p.Group,
