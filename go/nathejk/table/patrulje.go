...
func (c *patrulje) Consumes() (subjs []streaminterface.Subject) {
	return []streaminterface.Subject{
		...
		//streaminterface.SubjectFromStr("NATHEJK:2024.patrulje.*.updated"),
		//streaminterface.SubjectFromStr("NATHEJK:2024.patrulje.*.signedup"),
	}
}
	...
	*/
	...
	case "monolith:nathejk_team":
		...
		if body.Entity.DeletedUts.Time() == nil {
			...
			query := "INSERT INTO patrulje SET teamId=%q, year=\"%d\", teamNumber=%q, name=%q, groupName=%q, korps=%q, memberCount=%d, contactName=%q, contactPhone=%q, contactEmail=%q, signupStatus=%q, photoId=%q ON DUPLICATE KEY UPDATE teamNumber=VALUES(teamNumber), name=VALUES(name), groupName=VALUES(groupName), korps=VALUES(korps), memberCount=VALUES(memberCount), contactName=VALUES(contactName), contactPhone=VALUES(contactPhone), contactEmail=VALUES(contactEmail), signupStatus=VALUES(signupStatus), photoId=VALUES(photoId)"
			args := []any{
				body.Entity.ID,
